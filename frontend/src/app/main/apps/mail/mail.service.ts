...
export class MailService implements Resolve<any>
{
    ...
    replyMail(mail): Promise<any>{
        return new Promise((resolve, reject) => {
            ...
            this._httpClient.post(environment.url + 'tickets/admin/response',mail)
                .subscribe(response => {
    ...
    {
        return new Promise((resolve, reject) => {
                    this._httpClient.get(environment.url + 'tickets')
                        .subscribe((mails: any) => {

                            this.mails = mails._embedded.tickets.reverse()

                            this.mails = FuseUtils.filterMailArrayByString(this.mails, this.searchText);
    ...
    /* getMailNbr(){
        this._httpClient.get(environment.url + 'tickets')
                        .subscribe((mails: any) => {
                            for(var i = 0 ; i < mails._embedded.tickets.length ; i++){
                                if(mails._embedded.tickets[i].readAtByAdmin===null){
                                    this.newMails++;
                                }
                            }
                        })
